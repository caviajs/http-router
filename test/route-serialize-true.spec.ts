...
it('should correctly serialize true returned by handler', async () => {
  // sync
  {
    ...
    expect(response.headers['content-length']).toBe(Buffer.byteLength(JSON.stringify(EXAMPLE_TRUE)).toString());
...
it('should correctly overwrite the inferred content-type header after true serialization', async () => {
  const httpRouter: HttpRouter = new HttpRouter();
